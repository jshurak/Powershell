Function script-nbu
{
        ...
        Process
        {
            ...
            $DBList = get-content "d:\util\DatabaseRestoreList.txt"
            ...
            while($LastSundayOfTheMonth -eq $null){
                if((get-date -Year (get-date).Year -Month ($MostRecentSunday).Month -Day $DaysInMonth).DayOfWeek -eq 0)
                {
                    $LastSundayOfTheMonth = (get-date -Year (get-date).Year -Month ($MostRecentSunday).Month -Day $DaysInMonth)
                }
            ...
            $PolicyHash = New-Object System.Collections.Specialized.OrderedDictionary
            ...
            $Path = "C:\Program Files\VERITAS\NetBackup\bin\bplist.exe"
            $BatchPath = 'C:\Program Files\VERITAS\NetBackup\DbExt\MsSql\Daily_Restore.bch'
            $NBUMaster = 'RDGNBMS01'
            ...
            $PolicyHash.Keys | Sort-Object -Descending | % {
